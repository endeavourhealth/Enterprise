...
(function () {
	...
	flowchart.NodeViewModel = function (nodeDataModel) {
		...
		this.verb = createConnectorsViewModel(this.data.verb, 0, this);
		this.nextStep = createConnectorsViewModel(this.data.nextStep, flowchart.nodeWidth, this);
		...
		this.addInputConnector = function (connectorDataModel) {
			...
			this._addConnector(connectorDataModel, 0, this.data.verb, this.verb);
		};
		...
		this.addOutputConnector = function (connectorDataModel) {
			...
			this._addConnector(connectorDataModel, flowchart.nodeWidth, this.data.nextStep, this.nextStep);
		};
	};
	...
	flowchart.ChartViewModel = function (chartDataModel) {
		...
		this.findInputConnector = function (nodeID, connectorIndex) {
			...
			if (!node.verb || node.verb.length <= connectorIndex) {
				throw new Error("Node " + nodeID + " has invalid input connectors.");
			}

			return node.verb[connectorIndex];
		};
		...
		this.findOutputConnector = function (nodeID, connectorIndex) {
			...
			if (!node.nextStep || node.nextStep.length <= connectorIndex) {
				throw new Error("Node " + nodeID + " has invalid output connectors.");
			}

			return node.nextStep[connectorIndex];
		};
		...
		this.createNewConnection = function (sourceConnector, destConnector) {
			...
			var sourceConnectorIndex = sourceNode.nextStep.indexOf(sourceConnector);
			if (sourceConnectorIndex == -1) {
				sourceConnectorIndex = sourceNode.verb.indexOf(sourceConnector);
				if (sourceConnectorIndex == -1) {
					throw new Error("Failed to find source connector within either verb or nextStep of source node.");
				}
			}

			var destNode = destConnector.parentNode();
			var destConnectorIndex = destNode.verb.indexOf(destConnector);
			if (destConnectorIndex == -1) {
				destConnectorIndex = destNode.nextStep.indexOf(destConnector);
				if (destConnectorIndex == -1) {
					throw new Error("Failed to find dest connector within verb or ouputConnectors of dest node.");
				}
			}
